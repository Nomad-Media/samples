...
METHOD: POST
BODY: `+JSON.stringify(i,null,4));try{var o=await fetch(e,{method:"POST",headers:n,body:JSON.stringify(i)});if(!o.ok)throw await o.json()}catch(t){_apiExceptionHandler(t,"Upload Part Complete Failed")}}import{readFile}from"fs/promises";async function _uploadPart(t,i,n,a=3){for(let e=0;e<a;e++)try{var o=t.slice(i.startingPostion,i.endingPosition+1),r=new Headers,s=(r.append("Accept","application/json, text/plain, */*"),n&&console.log(`URL: ${i.url}
METHOD: POST`),await fetch(i.url,{method:"PUT",headers:r,body:o}));if(s.ok)return s.headers.get("ETag");throw await s.json()}catch(t){if(!(e<a-1)){_apiExceptionHandler(t,"Upload Part Failed");break}console.error(`Upload attempt ${e+1} failed. Retrying...`)}return null}async function _addCustomProperties(t,e,i,n,a,o,r){e=e+"/admin/asset/"+i,i=new Headers,i.append("Content-Type","application/json"),i.append("Authorization","Bearer "+t),t={displayName:n,displayDate:a,customProperties:o};r&&console.log(`URL: ${e}
METHOD: PATCH
...
METHOD: POST
BODY: `+JSON.stringify(t,null,4));try{var f=await fetch(e,{method:"POST",headers:h,body:JSON.stringify(t)});if(f.ok)return await f.json();throw await f.json()}catch(t){_apiExceptionHandler(t,"Search Failed")}}function _apiExceptionHandler(t,e){if(t)throw"string"==typeof t?new Error(e+": "+t):t.errors&&0!=t.errors.length&&t.errors.contentDefinitionId?new Error(`${e}: ${t.message}: `+t.errors.contentDefinitionId[0]):new Error(e+": "+t.message);throw new Error(e)}import{getRandomValues}from"crypto";function _newGuid(){return([1e7]+-1e3+-4e3+-8e3+-1e11).replace(/[018]/g,t=>(t^getRandomValues(new Uint8Array(1))[0]&15>>t/4).toString(16))}function _printDatetime(t){var e=(new Date).toLocaleTimeString();console.log(e+": "+t)}async function _sleep(e){if(e&&0!==e)return new Promise(t=>{setTimeout(()=>{t()},1e3*e)})}function _slugify(t){return t&&0!==t.trim().length?t.toString().normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().trim().replace(/\s+/g,"-").replace(/[^\w-]+/g,"").replace(/--+/g,"-"):""}async function _changeEmail(t,e,i,n,a){var e=e+"/account/change-email",o=new Headers,t=(o.append("Content-Type","application/json"),o.append("Authorization","Bearer "+t),{password:n,newEmail:i});a&&console.log(`URL: ${e}
METHOD: POST
